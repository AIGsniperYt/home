...
// Function to center the wrapper within the plane
function centerWrapper2() {
    const plane2Rect = plane2.getBoundingClientRect();
    const wrapper2Rect = wrapper2.getBoundingClientRect();

    const centerX = plane2Rect.left + plane2Rect.width / 2 - wrapper2Rect.width / 2;
    const centerY = plane2Rect.top + plane2Rect.height / 2 - wrapper2Rect.height / 2;

    wrapper2.style.left = centerX + 'px';
    wrapper2.style.top = centerY + 'px';
    requestAnimationFrame(centerWrapper2)
}
...
function updateHealthBars() {
    updateHealthBarPosition(healthBar1, wrapper1);
    updateHealthBarPosition(healthBar2, wrapper2);

    // Update health text
    healthText1.textContent = "Health: " + health1;
    healthText2.textContent = "Health: " + health2;

    // Ensure health1 doesn't go below 0
    if (health1 <= 0) {
        health1 = 0;
    }
    
    // Ensure health2 doesn't go below 0
    if (health2 <= 0) {
        health2 = 0;
    }
...
function updateHealthBarPosition(healthBar, wrapper) {
    const wrapperRect = wrapper.getBoundingClientRect();
    healthBar.style.left = wrapperRect.left + wrapperRect.width / 2 - healthBar.clientWidth / 2 + "px";
    healthBar.style.top = wrapperRect.top - 50 + "px";
}
...
function shootBullet1() {
    ...
    if (gameStarted) {
        ...
        function moveBullet() {
            ...
            bulletX = Math.round(parseFloat(bulletClone.style.left) || 0);
            console.log(bulletX)
